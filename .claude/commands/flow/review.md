...
- if the PR already carries review threads, verify them before looking for anything new: for each unresolved one, read the reply and the commit it names, then confirm against the code that the finding is actually gone
- resolve a thread only when the code proves it, never on the author's word — a reply claiming a fix is a claim, and rubber-stamping it makes this loop certify its own homework
- where the claimed fix does not hold, or holds only partly, say which in the thread and leave it unresolved; where nobody replied at all, leave it alone — nothing has changed to report
...
- thread ids and resolution state are GraphQL-only: query `reviewThreads` on the pull request for each thread's `id`, `isResolved`, `isOutdated` and its comments, and resolve with the `resolveReviewThread` mutation against that `id`. REST has neither the field nor the mutation
