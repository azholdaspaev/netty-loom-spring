...
- choose the path from the diff. The check path is for a diff with no behaviour to break: every changed file is a document nothing loads at run time, a file under `<module>/src/test/` from which the diff deletes no test and no assertion, or a `.java` file under `<module>/src/main/`, where `<module>` is one `settings.gradle.kts` includes, and every changed `src/main` line is a comment, javadoc, or an identifier renamed at every use that nothing binds by name at run time — not a `@ConfigurationProperties` component, a key `docs/configuration.md` lists, or a name Spring, Servlet or Netty looks up. A diff that changes any other file — anything under `.claude/` or `.github/`, a script, a build file, a workflow, a resource, a source outside those modules that `./gradlew build` never compiles — and any diff you are unsure of, takes the mutation path (#403)
- check path: run `./gradlew build`, then `.claude/scripts/check-naming.sh` over the touched `.java` files, skipped when none changed — a failure is a finding when the diff touched the declaration it reports, an annotation on it, or the other declaration a case collision names; any other predates the PR. Drop a finding already raised on the PR (`.claude/scripts/pr-comments.sh $1`), as the review stage does. Rerun any ROUNDS-based test the diff touches as the mutation path does. Mutate only to prove, as the mutation path does, that each test binds whose code the diff adds or changes beyond a rename or a comment — its method, or a helper, setup, fixture, resource or argument source it runs — and no other test
- mutation path:
