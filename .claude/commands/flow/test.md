...
- choose the path from the diff. The check path is for a diff with no behaviour to break: every changed file is a document nothing loads at run time, a file under `<module>/src/test/`, or a `.java` file under `<module>/src/main/`, where `<module>` is one `settings.gradle.kts` includes, and every changed `src/main` line is a comment, javadoc, or an identifier renamed at every use that nothing binds by name at run time — not a `@ConfigurationProperties` component, a key `docs/configuration.md` lists, or a name Spring, Servlet or Netty looks up. A diff that changes any other file — anything under `.claude/` or `.github/`, a script, a build file, a workflow, a resource, a source outside those modules that `./gradlew build` never compiles — and any diff you are unsure of, takes the mutation path (#403)
