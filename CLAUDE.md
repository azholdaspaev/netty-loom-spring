...
`.github/workflows/publish.yml` publishes to Sonatype. Snapshots go out on `workflow_dispatch` only — deliberately not on push to `main`, since a snapshot is disposable and an automatic one would redden `main` for as long as the Portal token is missing. A `v*.*.*` tag runs the release path: the tag supplies the version and is checked against `gradle.properties`, `build` runs before anything is uploaded, and the staged artifacts are zipped (minus every `maven-metadata.xml`, which the Portal rejects) and POSTed with `publishingType=USER_MANAGED`, so the job stops at `VALIDATED` and a human presses Publish. Neither job is a required status check.
