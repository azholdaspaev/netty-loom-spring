...
| Module | Role |
| --- | --- |
| `netty-loom-spring-core` | Pure Netty, no Spring. `NettyServer` manages lifecycle; `NettyServerChannelInitializer` delegates pipeline setup to the `NettyPipelineConfigurer` SPI. Netty transport, codec-http, handler, native epoll/kqueue |
...
**Seams:** `core.handler.HttpRequestDispatcher` separates the Netty pipeline from any higher-layer dispatcher and keeps `core` free of Spring; `core.pipeline.NettyPipelineConfigurer` owns pipeline assembly. README.md § Architecture has the request flow and the pipeline handler names.
...
`AGENTS.md` names the commit, pull request and issue templates, and is normative for their use. `.githooks/commit-msg` enforces the commit shape.

## CI

`.github/workflows/build.yml` runs `./gradlew build` on ubuntu and macos; both matrix cells must pass to merge. Everything else about the workflows and rulesets loads from `.claude/rules/github-workflows.md` when a file under `.github/` is read.

## Code Review

Review runs in two passes. The bug pass (`/code-review`) is tuned for correctness recall: it requires a concrete failure scenario per finding and discards style, quality and state-dependent findings. The maintainability pass is the `maintainability-pass` skill, which owns the lenses for naming consistency, magic constants, duplication, comment budget, simplicity and scope, and module boundaries. State-dependent correctness — concurrency, lifecycle, time arithmetic — is rule 6 below, so that either pass can quote it. `/flow:review` runs both locally; `.github/workflows/claude-review.yml` runs them on the `review/claude` label.
