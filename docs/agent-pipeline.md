...
1. **A question.** A stage posts one comment starting with `<!-- agent:question -->` and the
   issue moves to `agent/needs-input`. Reply on the issue; the next tick moves it back to
   `agent/queued` and the pipeline starts over on the same worktree — at the review stage once a
   pull request exists.
