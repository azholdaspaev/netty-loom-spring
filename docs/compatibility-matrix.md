...
| `Filter.destroy()`, `Servlet.destroy()` | `works` | On shutdown and on `ApplicationContext.stop()`: `DispatcherServlet` first, then the filters in reverse registration order, then session teardown, then `contextDestroyed`. A `start()` re-runs `init()` on each. A `destroy()` that throws is logged and the pass continues. **Constraint:** a `dispatcherServlet` bean built as `new DispatcherServlet(applicationContext)` owns that context by Spring's contract, so its `destroy()` closes it (`FrameworkServlet.destroy()`), and on a plain `stop()` that closes the whole application — Tomcat only destroys the servlet after the context has closed, where the same call is a no-op. Boot's default `new DispatcherServlet()` is injected instead and is unaffected; there is no accessor for the ownership flag, so this is not detected |
