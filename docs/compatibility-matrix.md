...
| `Filter.destroy()`, `Servlet.destroy()` | `works` | On shutdown and on `ApplicationContext.stop()`: `DispatcherServlet` first, then the filters in reverse registration order, then session teardown, then `contextDestroyed`. A `start()` re-runs `init()` on each. A `destroy()` that throws is logged and the pass continues |
