...
| Body consumption by `getParameter` | `works` | Reading form parameters consumes the body, as on Tomcat — unless `getInputStream()` or `getReader()` has already claimed it, which stops the parse exactly as Tomcat's `Request.doParseParameters` does. Bounded separately at 2 MiB, Tomcat's `maxPostSize` default; past that the request is answered `413` |
...
| Request body size | `partial` | Streamed, never held whole: undrained bytes per exchange are bounded at 64 KiB and the total is capped at 1 MiB, answered `413` ([#42](https://github.com/azholdaspaev/netty-loom-spring/issues/42) makes the cap a property) |
