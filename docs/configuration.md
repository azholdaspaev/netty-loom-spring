...
| `server.netty.accept-count` | `int` | `128` | Listen backlog (`SO_BACKLOG`): how many connections that have completed the TCP handshake may queue before the boss loop accepts them. Same concept as `server.tomcat.accept-count`, with one difference at `0`: Tomcat and the `nio` transport bind through the JDK, which substitutes `50` for any value below `1`, while epoll and kqueue pass `0` to `listen()` verbatim — a queue of at most one pending connection on Linux. Otherwise the effective depth is `min(value, net.core.somaxconn)` on Linux, and the kernel clamps silently — `8192` where `somaxconn` is `4096` yields `4096`, with no error and no log. `ss -tln` shows the effective depth as `Send-Q` on the listening socket |
