...
drift is silent and permanent. Two consequences follow:
...
some files, zero in others. That is why rule 5 overrides rule 3 (match existing style) for comment
volume, and only for comment volume.
