...
See [`results/SNAPSHOT.md`](results/SNAPSHOT.md) for the last snapshot the harness's own machine
produced, with the exact machine and JVM flags. Regenerate it by re-running `scripts/run-all.sh`.

The analysed sweeps live in [`../docs/benchmarks/`](../docs/benchmarks), one directory per date. The
newest, [2026-08-23](../docs/benchmarks/2026-08-23/COMPARISON.md), is the first on dedicated
hardware and the one the root README quotes; each `COMPARISON.md` says what it may be compared with.
